...
package {
	...
	public class SWFUpload extends Sprite {
		...
		*************************************************************** */
		...
		private function StartPreview(file_id:String = "", resizeSettings:Object = null):void {
			var file_index:Number = this.FindIndexInFileQueue(file_id);
			if (file_index >= 0) {
				var file:FileItem = FileItem(this.file_queue[file_index]);
				var resizer:ImageResizer = new ImageResizer(file, resizeSettings["width"], resizeSettings["height"], resizeSettings["encoding"], resizeSettings["quality"], resizeSettings["allowEnlarging"]);
				resizer.addEventListener(ImageResizerEvent.COMPLETE, this.PreviewCompleteHandler);
				resizer.addEventListener(ErrorEvent.ERROR, this.PreviewErrorHandler);
				...
				this.Debug("StartPreview(): not found file.");
			}
		}
		
		private function PreviewCompleteHandler(event:ImageResizerEvent):void {
			this.Debug("StartPreview Complete!");
			var imageBase64:String = convertAvatartoString(event.data);
			ExternalCall.PreviewComplete(imageBase64);
			this.Debug("Image Base64:"+imageBase64);
		}
		
		private function PreviewErrorHandler(event:ImageResizerEvent):void {
