var Drag={
	...
	"drag":function(e){
		...
		var top=parseInt(handle.root.style.top);
		var left=parseInt(handle.root.style.left);
		var width = $(handle.root).width();
		var height = $(handle.root).height();
		...
		currentLeft = (currentLeft+width)>pWidth?(pWidth-width):currentLeft;
		currentTop = currentTop<0?0:currentTop;
		currentTop = (currentTop+height)>pHeight?(pHeight-height):currentTop;
