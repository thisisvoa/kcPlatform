...
("="+b.async.otherParam[e+1]).replace(/&/g,"%26")+"&";else for(var q in b.async.otherParam)j?(m=typeof b.async.otherParam[q]=="string"?'"':"",h+='"'+q+('":'+m+b.async.otherParam[q]).replace(/'/g,"\\'")+m+","):h+=q+("="+b.async.otherParam[q]).replace(/&/g,"%26")+"&";h += j?("$currentTime$:"+new Date().getTime()+","):("$currentTime$="+new Date().getTime()+"&");h.length>1&&(h=h.substring(0,h.length-1));j&&(h+="}");l.ajax({contentType:b.async.contentType,type:b.async.type,url:k.apply(b.async.url,[b.treeId,a],b.async.url),data:h,dataType:b.async.dataType,success:function(e){var g=[];try{g=!e||e.length==
