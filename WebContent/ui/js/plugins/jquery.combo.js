(function($) {
	...
	function bindEvents(jq) {
		...
		if (!options.disabled) {
			...
			textBox.bind("click.combo", function(){
				if (panel.is(":visible")) {
					hidePanel(jq);
				} else {
				...
				combo.addClass("combo_hover");
			},function(){
				combo.removeClass("combo_hover");
			}).focus(function(){
				combo.addClass("combo_click");
			}).blur(function(){
				combo.removeClass("combo_click");
			});
	...
	$.fn.combo.parseOptions = function(target) {
		...
		return $.extend({}, {
				...
				onChange:(t.attr("onChange")?eval(t.attr("onChange")):undefined)
			});
	};
	/**
	 * 默认参数
