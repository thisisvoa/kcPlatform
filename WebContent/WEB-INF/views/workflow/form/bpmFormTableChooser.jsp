...
function radioFormat(cellVal,options,rowd){
	return "<input id=\"ra_"+rowd.tableId+"\" type=\"radio\" name=\"tableSelectRadio\" value=\""+rowd.tableId+"\" onclick=\"gridRadioCheck('"+rowd.tableId+"')\">";
}
function gridRadioCheck(tableId){
	var rowd = $("#bpmFormTableList").grid("getRowData", tableId);
	table = rowd;
}

function doSave(){
	if(table==null){
		MessageUtil.alert("请选择业务表!");
		return;
	}else{
...
<ui:LayoutContainer fit="true">
	...
	<ui:Layout region="center">
		<ui:Grid id="bpmFormTableList" datatype="json" shrinkToFit="true" fit="true" viewrecords="true" pageable="true"
				url="${ctx}/workflow/formTable/list.html?isMain=1" multiselect="false" rownumbers="false"
