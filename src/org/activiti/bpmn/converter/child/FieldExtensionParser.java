...
public class FieldExtensionParser extends BaseChildElementParser {
  ...
  public void parseChildElement(XMLStreamReader xtr, BaseElement parentElement, BpmnModel model) throws Exception {
    ...
    if (parentElement instanceof ActivitiListener == false && parentElement instanceof ServiceTask == false && 
        parentElement instanceof SendTask == false && parentElement instanceof UserTask == false && 
        parentElement instanceof CallActivity == false) return;//modify by linbs 20131230 对UserTask、CallActivity添加扩展字段解析
    ...
    if (parentElement instanceof ActivitiListener) {
      ...
      ((UserTask) parentElement).getFieldExtensions().add(extension);//modify by linbs 20131230 对UserTask添加扩展字段解析
    } else if (parentElement instanceof CallActivity) {
      ((CallActivity) parentElement).getFieldExtensions().add(extension);//modify by linbs 20131230 对CallActivity添加扩展字段解析
    } else {
