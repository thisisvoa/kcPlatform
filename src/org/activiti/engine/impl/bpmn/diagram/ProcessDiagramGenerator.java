...
//    // Draw pool shape, if process is participant in collaboration
    for (Pool pool : bpmnModel.getPools()) {
      ...
      if(graphicInfo.isHorizontal()){
	      processDiagramCanvas.drawHPool(pool.getName(), (int) graphicInfo.getX(), (int) graphicInfo.getY(), 
	              (int) graphicInfo.getWidth(),  (int) graphicInfo.getHeight());
      }else{
    	  processDiagramCanvas.drawVPool(pool.getName(), (int) graphicInfo.getX(), (int) graphicInfo.getY(), 
	              (int) graphicInfo.getWidth(),  (int) graphicInfo.getHeight());
      }
    }
    ...
    for (Process process : bpmnModel.getProcesses()) {
      for (Lane lane : process.getLanes()) {
        ...
        if(graphicInfo.isHorizontal()){
        	processDiagramCanvas.drawHLane(lane.getName(), (int) graphicInfo.getX(), (int) graphicInfo.getY(), 
                    (int) graphicInfo.getWidth(),  (int) graphicInfo.getHeight());
        }else{
        	processDiagramCanvas.drawVLane(lane.getName(), (int) graphicInfo.getX(), (int) graphicInfo.getY(), 
                    (int) graphicInfo.getWidth(),  (int) graphicInfo.getHeight());
        }
      }
    }
  ...
  public static InputStream generateDiagram(BpmnModel bpmnModel, Map<String, String> highLightedActivities, String imageType) {
	ProcessDiagramCanvas processDiagramCanvas = initProcessDiagramCanvas(bpmnModel);
	for (FlowNode flowNode : bpmnModel.getProcesses().get(0).findFlowElementsOfType(FlowNode.class)) {
			drawActivity(processDiagramCanvas, bpmnModel, highLightedActivities, flowNode);
	}
	return processDiagramCanvas.generateImage(imageType);
  }
  
  protected static void drawActivity(ProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, Map<String, String> highLightedActivities, FlowNode flowNode) {
	    ...
	    if (flowNode instanceof FlowElementsContainer) {
	      for (FlowElement nestedFlowElement : ((FlowElementsContainer) flowNode).getFlowElements()) {
	        if (nestedFlowElement instanceof FlowNode) {
	          drawActivity(processDiagramCanvas, bpmnModel, highLightedActivities, (FlowNode)nestedFlowElement);
	        }
	      }
	    }
  }
  
  private static void drawHighLight(ProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo, String color) {
  ...
  protected static void drawActivity(ProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode, List<String> highLightedActivities, List<String> highLightedFlows) {
    ...
    if (drawInstruction != null) {
      ...
      if (highLightedActivities.contains(flowNode.getId())) {
        drawHighLight1(processDiagramCanvas, bpmnModel.getGraphicInfo(flowNode.getId()));
      }
...
//    }
  ...
  private static void drawHighLight1(ProcessDiagramCanvas processDiagramCanvas, GraphicInfo graphicInfo) {
