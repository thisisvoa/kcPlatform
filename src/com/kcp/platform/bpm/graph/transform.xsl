...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" 
	...
	xmlns:calc="xalan://com.kcp.platform.bpm.graph.TransformUtil" 
	...
	<xsl:template match="/">
		...
		xmlns:omgdc="http://www.omg.org/spec/DD/20100524/DC" 
		xmlns:omgdi="http://www.omg.org/spec/DD/20100524/DI" 
		xmlns:ht="http://www.kcp.com/BPMN20EXT"
		xsi:schemaLocation="http://www.kcp.com/BPMN20EXT BPMN20EXT.xsd"
	...
	<xsl:template match="/diagram">
		...
		<xsl:if test="contains($comp,'Event') or contains($comp,'Task') or contains($comp,'SequenceFlow') or contains($comp,'SubProcess') or contains($comp,'Gateway')">
			<process>
				...
				<extensionElements>
					<!-- 流程开始事件监听器 -->
					<activiti:executionListener class="com.kcp.platform.bpm.listener.StartEventListener" event="start" />
					<!-- 流程结束事件监听器 -->
					<activiti:executionListener class="com.kcp.platform.bpm.listener.EndEventListener" event="end" />
				</extensionElements>
	...
	<xsl:template match="bg:SubProcess">
		<xsl:for-each select=".">
			<subProcess>
				...
				<extensionElements>
					<!-- 流程开始事件监听器 -->
					<activiti:executionListener class="com.kcp.platform.bpm.listener.SubProcessStartListener" event="start" />
					<!-- 流程结束事件监听器 -->
					<activiti:executionListener class="com.kcp.platform.bpm.listener.SubProcessEndListener" event="end" />
				</extensionElements>
			...
			<xsl:choose>
				<xsl:when test="@user='true'">
					<userTask>
						...
						<extensionElements>
							<activiti:taskListener event="create" class="com.kcp.platform.bpm.listener.TaskCreateListener" />
							<activiti:taskListener event="assignment" class="com.kcp.platform.bpm.listener.TaskAssignListener" />
							<activiti:taskListener event="complete" class="com.kcp.platform.bpm.listener.TaskCompleteListener" />
				...
				<xsl:when test="@script='true'">
					<serviceTask activiti:class="com.kcp.platform.bpm.delegate.ScriptTask">
						<xsl:call-template name="setAttrubute">
							<xsl:with-param name="obj" select="."></xsl:with-param>
						</xsl:call-template>
						<extensionElements>
							<activiti:executionListener class="com.kcp.platform.bpm.listener.AutoTaskListener" event="start" />
						</extensionElements>
					</serviceTask>
				</xsl:when>
				...
				<xsl:when test="@multiInstance='true'">
					<userTask>
						...
						<extensionElements>
							<activiti:taskListener event="create" class="com.kcp.platform.bpm.listener.TaskSignCreateListener" />
							<activiti:taskListener event="assignment" class="com.kcp.platform.bpm.listener.TaskAssignListener" />
							<activiti:taskListener event="complete" class="com.kcp.platform.bpm.listener.TaskCompleteListener" />
							<xsl:call-template name="setTaskExtProperties">
								<xsl:with-param name="obj" select="."></xsl:with-param>
